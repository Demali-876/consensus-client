...
class ConsensusSDK {
  ...
  async registerWithProxy(walletName, accountAddress, cdpAccount) {
    ...
    try {
      // Export private key using wallet name
      const cdp = new CdpClient();
      const privateKeyData = await cdp.evm.exportAccount({ name: walletName });
      
      // Debug: log the format we received
      console.log('Debug - privateKeyData:', typeof privateKeyData, privateKeyData);
      
      // Handle different possible formats
      let privateKey;
      if (typeof privateKeyData === 'string') {
        privateKey = privateKeyData.startsWith('0x') ? privateKeyData : `0x${privateKeyData}`;
      } else if (privateKeyData && privateKeyData.privateKey) {
        privateKey = privateKeyData.privateKey.startsWith('0x') ? privateKeyData.privateKey : `0x${privateKeyData.privateKey}`;
      } else {
        throw new Error('Unexpected private key format from CDP');
      }
      
      console.log('Debug - formatted privateKey:', privateKey);
      ...
      if (error.message.includes('ECONNREFUSED')) {
        throw new Error('Cannot connect to x402 proxy. Is it running on ' + this.x402ProxyUrl + '?');
      }
  ...
  async setup() {
    try {
      console.log(chalk.blue.bold('🏗️  Consensus SDK Setup\n'));
      ...
      const { wallet_name, account_address, cdpAccount } = await this.createWallet();
      const api_key = await this.registerWithProxy(wallet_name, account_address, cdpAccount);
      ...
      console.log(`  -d '{"target_url": "https://api.example.com", "idempotency_key": "unique-key"}'`);
  ...
  showHelp() {
    ...
    console.log('  X402_PROXY_URL (optional, defaults to http://localhost:3001)');
